        ...
        if ("create" === data.type) {
            if (RECORDRTP) {
              ...
              postMessage({ type: "outputData", buf: data.buf, dType: 1 }, [data.buf]);
                ...
                if (iRet != 0) {
                    if(iRet == -2147483647) {
                      postMessage({ type: "outputData", dType: 1501 }); //标记为格式不支持
                    } else {
                      postMessage({ type: "outputData", dType: 1501 }); //转封装创建失败，也同样提示码流格式不支持，如果后续要细化再区分
                    }
            ...
            if (RECORDRTP) {
              ...
              if (data.samplingParam) {
                ...
                if (pp == -2147483627) {
                  ...
                  postMessage({ type: "outputData", dType: 1500 }); //标记为密码错误
